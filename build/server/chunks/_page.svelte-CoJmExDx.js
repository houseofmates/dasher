...
import { q as ArrowLeft, r as Trash, H as HardDrive, w as Calendar, x as Database } from './lib--dU9BMBy.js';
...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		$$renderer.push(`<!----></div> <div><p class="text-[10px] text-white/20 uppercase font-bold tracking-widest">driver</p> <p class="font-mono text-sm">${escape_html(volume().Driver)}</p></div></div> <div class="flex items-center gap-3"><div class="p-2 bg-accent-blue/10 rounded-lg text-accent-blue">`);
		Calendar($$renderer, { size: 24 });
		$$renderer.push(`<!----></div> <div><p class="text-[10px] text-white/20 uppercase font-bold tracking-widest">created</p> <p class="font-mono text-sm">${escape_html(volume().CreatedAt ? formatDate(volume().CreatedAt) : "n/a")}</p></div></div> <div class="flex items-center gap-3"><div class="p-2 bg-accent-green/10 rounded-lg text-accent-green">`);
		Database($$renderer, { size: 24 });
		$$renderer.push(`<!----></div> <div><p class="text-[10px] text-white/20 uppercase font-bold tracking-widest">scope</p> <p class="font-mono text-sm uppercase">${escape_html(volume().Scope)}</p></div></div></div> <div class="card space-y-4"><p class="text-xs font-bold text-white/40 uppercase tracking-widest">mountpoint</p> <div class="bg-black/20 p-4 rounded-xl border border-white/5 font-mono text-xs break-all leading-relaxed">${escape_html(volume().Mountpoint)}</div></div></div> <div class="lg:col-span-2 space-y-6"><div class="card h-full flex flex-col gap-4 min-h-[400px]"><div class="flex items-center justify-between"><p class="text-xs font-bold text-white/40 uppercase tracking-widest">labels &amp; options</p></div> `);
