import { e as ensure_array_like, c as escape_html, K as derived } from './index-server-Ip8I2Crl.js';
import { q as ArrowLeft, r as Trash, I as Info, t as Tag } from './lib--dU9BMBy.js';
...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		if (image()) {
			...
			$$renderer.push(`<!--]--></div></div> <div class="card space-y-4"><h3 class="text-xl font-bold">configuration</h3> <div class="grid grid-cols-1 md:grid-cols-2 gap-4"><div class="space-y-1"><p class="text-[10px] text-white/40 uppercase tracking-widest font-bold">architecture</p> <p class="font-mono text-sm">${escape_html(image().Architecture)}</p></div> <div class="space-y-1"><p class="text-[10px] text-white/40 uppercase tracking-widest font-bold">os</p> <p class="font-mono text-sm">${escape_html(image().Os)}</p></div> <div class="space-y-1"><p class="text-[10px] text-white/40 uppercase tracking-widest font-bold">docker version</p> <p class="font-mono text-sm">${escape_html(image().DockerVersion)}</p></div> <div class="space-y-1"><p class="text-[10px] text-white/40 uppercase tracking-widest font-bold">author</p> <p class="font-mono text-sm truncate">${escape_html(image().Author || "N/A")}</p></div></div></div> `);
			...
			$$renderer.push(`<!----> <span class="font-bold uppercase text-xs tracking-widest">quick info</span></div> <div class="space-y-4 pt-2"><div class="flex justify-between items-center"><span class="text-white/40 text-sm">Size</span> <span class="font-mono">${escape_html(formatSize(image().Size))}</span></div> <div class="flex justify-between items-center"><span class="text-white/40 text-sm">Created</span> <span class="font-mono">${escape_html(new Date(image().Created).toLocaleDateString())}</span></div> <div class="flex justify-between items-center"><span class="text-white/40 text-sm">Layers</span> <span class="font-mono">${escape_html(image().RootFS?.Layers?.length || 0)}</span></div></div></div> `);
			if (image().Config && image().Config.ExposedPorts) {
				...
				$$renderer.push(`<!----> <span class="font-bold uppercase text-xs tracking-widest">exposed ports</span></div> <div class="flex flex-wrap gap-2 pt-2"><!--[-->`);
...
//# sourceMappingURL=_page.svelte-CtJiz7ep.js.map
