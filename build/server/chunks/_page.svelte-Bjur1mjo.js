import { c as escape_html, e as ensure_array_like, K as derived } from './index-server-Ip8I2Crl.js';
import { q as ArrowLeft, r as Trash, S as ShareNetwork, G as Globe, u as ShieldCheck, b as Cube } from './lib--dU9BMBy.js';
...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		$$renderer.push(`<!----></div> <div><p class="text-[10px] text-white/20 uppercase font-bold tracking-widest">driver</p> <p class="font-mono text-sm uppercase">${escape_html(network().Driver)}</p></div></div> <div class="flex items-center gap-3"><div class="p-2 bg-accent-yellow/10 rounded-lg text-accent-yellow">`);
		Globe($$renderer, { size: 24 });
		$$renderer.push(`<!----></div> <div><p class="text-[10px] text-white/20 uppercase font-bold tracking-widest">scope</p> <p class="font-mono text-sm uppercase">${escape_html(network().Scope)}</p></div></div> <div class="flex items-center gap-3"><div class="p-2 bg-accent-green/10 rounded-lg text-accent-green">`);
		ShieldCheck($$renderer, { size: 24 });
		$$renderer.push(`<!----></div> <div><p class="text-[10px] text-white/20 uppercase font-bold tracking-widest">internal</p> <p class="font-mono text-sm">${escape_html(network().Internal ? "yes" : "no")}</p></div></div></div> <div class="card space-y-4"><p class="text-xs font-bold text-white/40 uppercase tracking-widest">ipam config</p> `);
		if (network().IPAM?.Config?.length > 0) {
			...
			for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
				...
				$$renderer.push(`<div class="space-y-2 p-3 bg-white/5 rounded-lg border border-white/5"><div class="flex justify-between"><span class="text-[10px] text-white/40 uppercase">subnet</span> <span class="text-xs font-mono">${escape_html(config.Subnet)}</span></div> <div class="flex justify-between"><span class="text-[10px] text-white/40 uppercase">gateway</span> <span class="text-xs font-mono">${escape_html(config.Gateway)}</span></div></div>`);
			}
			$$renderer.push(`<!--]-->`);
		} else {
			$$renderer.push("<!--[-1-->");
		...
		$$renderer.push(`<!--]--></div></div> <div class="lg:col-span-2 space-y-6"><div class="card flex-1 flex flex-col gap-4"><p class="text-xs font-bold text-white/40 uppercase tracking-widest">connected containers</p> `);
		...
		$$renderer.push(`<!--]--></div> <div class="card"><p class="text-xs font-bold text-white/40 uppercase tracking-widest mb-4">options</p> <div class="grid grid-cols-2 gap-4">`);
		...
		if (each_array_2.length !== 0) {
			...
			for (let $$index_2 = 0, $$length = each_array_2.length; $$index_2 < $$length; $$index_2++) {
				...
				$$renderer.push(`<div class="p-3 bg-black/20 rounded-lg border border-white/5"><p class="text-[9px] text-white/20 uppercase font-bold tracking-widest mb-1">${escape_html(key)}</p> <p class="text-xs font-mono text-white/60 truncate">${escape_html(val)}</p></div>`);
			}
		} else {
			$$renderer.push("<!--[!-->");
...
//# sourceMappingURL=_page.svelte-Bjur1mjo.js.map
