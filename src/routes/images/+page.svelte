...
<div class="space-y-8">
  ...
  {#if pullProgress.length > 0}
    <div class="card bg-black/40 border-accent-yellow/20 space-y-3" transition:slide>
      <div class="flex items-center justify-between">
        <p class="text-xs font-bold text-accent-yellow uppercase tracking-widest">pulling progress</p>
  ...
  <div class="bento-grid">
    {#each filteredImages as img (img.id)}
      <div 
        class="card group relative flex flex-col justify-between hover:border-white/20 transition-all cursor-pointer"
        in:scale={{ duration: 200, start: 0.95 }}
        out:fade={{ duration: 150 }}
        oncontextmenu={(e) => handleContextMenu(e, img)}
      >
        <a href="/images/{img.id}" class="absolute inset-0 z-0"></a>
        
        <div class="relative z-10 space-y-4 pointer-events-none">
          <div class="flex justify-between items-start">
            <div class="space-y-1 overflow-hidden pr-8">
              {#each img.tags as tag}
                <p class="font-bold text-lg leading-tight truncate preserve-case" title={tag}>{tag}</p>
              {/each}
              <p class="text-[10px] text-white/20 font-mono truncate uppercase tracking-tighter">{img.id.replace('sha256:', '').slice(0, 12)}</p>
            </div>
            <div class="text-white/20 group-hover:text-white/40 transition-colors pointer-events-auto">
               <button class="p-1 hover:text-white" onclick={(e) => { e.preventDefault(); handleContextMenu(e, img); }}>
                <DotsThreeVertical size={20} />
               </button>
            </div>
          </div>
          
          <div class="flex justify-between items-center text-[10px] text-white/40 font-mono uppercase tracking-widest pt-4 border-t border-white/5">
...
>
  {#if menu.image}
    <div class="px-4 py-2 border-b border-white/5 mb-1">
      <p class="text-[10px] font-bold text-white/20 uppercase tracking-widest truncate">image: {menu.image.tags[0] || 'untagged'}</p>
    </div>
    ...
    <ContextMenuItem label="tag image" icon={Tag} onclick={() => tagImage(menu.image.id)} />
