import { getUnreadNotifications, addNotification } from './db';
...
export function shouldShowBrowserNotification(type: string, message: string): boolean {
...
export function startNotificationPolling(callback: (notifications: any[]) => void, intervalMs = 30000) {
          ...
          newNotifications.forEach((notif: any) => {
            if (shouldShowBrowserNotification(notif.type, notif.message)) {
              showBrowserNotification(
                `Dasher Dashboard - ${notif.type.toUpperCase()}`,
                notif.message,
                undefined,
                () => {
