...
export async function createBackup(outputDir) {
    ...
    try {
        ...
        for (const vol of volumes) {
            try {
                ...
                if (vol.startsWith('buildx_') || vol.includes('kubelet'))
                    continue;
...
export async function restoreBackup(backupPath, restoreVolumes = true) {
    const restored = [];
