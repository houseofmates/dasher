...
export async function scanImage(imageName, imageId, forceRescan = false) {
    ...
    try {
        ...
        if (scanMethod === 'docker') {
            const { stdout: dockerOutput } = await execAsync(`docker run --rm -v /var/run/docker.sock:/var/run/docker.sock aquasec/trivy:latest image --format json --severity HIGH,CRITICAL,MEDIUM,LOW ${imageName}`, { timeout: 300000 });
            stdout = dockerOutput;
        }
        else {
            const { stdout: nativeOutput } = await execAsync(`trivy image --format json --severity HIGH,CRITICAL,MEDIUM,LOW ${imageName}`, { timeout: 300000 });
            stdout = nativeOutput;
        }
...
function parseTrivyOutput(stdout, imageName) {
    const result = {
        image: imageName,
        scannedAt: new Date().toISOString(),
        vulns: [],
