...
async function attemptReconnection(): Promise<void> {
  if (_isReconnecting || _circuitBreakerOpen) return;
  ...
  try {
    ...
    (global as any).__docker = newDocker;
    _isHealthy = true;
    _lastError = null;
...
export async function checkDockerHealth(): Promise<{ healthy: boolean; error?: string; latency: number; reconnecting?: boolean; circuitBreakerOpen?: boolean }> {
  ...
  try {
    ...
    const dockerInstance = (global as any).__docker || docker;
...
export function isDockerHealthy(): boolean {
  // If last check was > 30s ago, assume stale but don't block
  if (Date.now() - _lastCheck > 30000 && !_isHealthy) return false;
  return _isHealthy;
}
